# function.data.ipw.PersonTime.SWglmOutcome_Exposure_k.source.r
# https://github.com/mkim0710/causal-inference-book/blob/master/data/function.data.ipw.PersonTime.SWglmOutcome_Exposure_k.source.r
...
#@ nhefs.ipw.PersonTime.SWglmOutcome_Exposure_k ========
data = nhefs.ipw.PersonTime %>% select(Dk_plus1, Exposure, k, StabilizedWeight)
nhefs.ipw.PersonTime.SWglmOutcome_Exposure_k =
    glm(formula = Dk_plus1 ~ Exposure * (k + I(k^2)), data = data, weight = StabilizedWeight, family = binomial)
# > nhefs.ipw.PersonTime.SWglmOutcome_Exposure_k =
# +     glm(formula = Dk_plus1 ~ Exposure * (k + I(k^2)), data = nhefs.ipw.PersonTime, weight = StabilizedWeight, family = binomial)
# Warning message:
# In eval(family$initialize) : non-integer #successes in a binomial glm!

nhefs.ipw.PersonTime.SWglmOutcome_Exposure_k %>% {cbind( `exp(coef(.))` = exp(coef(.)), exp(confint.default(.)), `Pr(>|z|)` = summary(.)$coefficients[,"Pr(>|z|)"] )} %>% round(2) %>% as.data.frame %>% rownames_to_column %>% as.tibble #----
# > nhefs.ipw.PersonTime.SWglmOutcome_Exposure_k %>% {cbind( `exp(coef(.))` = exp(coef(.)), exp(confint.default(.)), `Pr(>|z|)` = summary(.)$coefficients[,"Pr(>|z|)"] )} %>% round(2) %>% as.data.frame %>% rownames_to_column %>% as.tibble #----
# # A tibble: 6 x 5
#   rowname         `exp(coef(.))` `2.5 %` `97.5 %` `Pr(>|z|)`
#   <chr>                    <dbl>   <dbl>    <dbl>      <dbl>
# 1 (Intercept)               0       0        0          0   
# 2 Exposure                  0.84    0.35     1.98       0.68
# 3 k                         1.02    1        1.04       0.02
# 4 I(k^2)                    1       1        1          0.06
# 5 Exposure:k                1.02    0.99     1.05       0.25
# 6 Exposure:I(k^2)           1       1        1          0.12
